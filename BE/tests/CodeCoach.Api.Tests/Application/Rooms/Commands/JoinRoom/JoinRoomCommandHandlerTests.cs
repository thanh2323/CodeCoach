...
public class JoinRoomCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenRoomAndUserAreValid_CreatesParticipantAndWorkspace()
    {
        ...
        _roomParticipantRepositoryMock.Verify(
            mock => mock.AddAsync(It.IsAny<RoomParticipant>(), It.IsAny<CancellationToken>()),
            Times.Once);
        _workspaceRepositoryMock.Verify(
            mock => mock.AddAsync(It.IsAny<Workspace>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }

    [Fact]
    public async Task Handle_WhenRoomDoesNotExist_ThrowsKeyNotFoundException()
    {
        _roomRepositoryMock
            .Setup(mock => mock.GetByJoinCodeAsync("MISSING", It.IsAny<CancellationToken>()))
            .ReturnsAsync((Room?)null);

        var action = () => _handler.Handle(new JoinRoomCommand("MISSING", Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_WhenUserDoesNotExist_ThrowsKeyNotFoundException()
    {
        ...
        var action = () => _handler.Handle(new JoinRoomCommand("JOIN01", userId), CancellationToken.None);

        await Assert.ThrowsAsync<KeyNotFoundException>(action);
    }

    [Fact]
    public async Task Handle_WhenRoomIsClosed_ThrowsInvalidOperationException()
    {
        var mentorId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var room = new Room("Algorithms", "JOIN01", mentorId, mentorId);
        ...
        typeof(Room).GetProperty(nameof(Room.Status), BindingFlags.Instance | BindingFlags.Public)!
            .SetValue(room, RoomStatus.Closed);
    ...
    [Fact]
    public async Task Handle_WhenUserAlreadyJoined_ThrowsInvalidOperationException()
    {
        var mentorId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var room = new Room("Algorithms", "JOIN01", mentorId, mentorId);
